...
description: "Use this skill when creating, modifying, reviewing, or testing the Vendra Currency module in app-modules/vendra-currency, or when creating future currency-like Filament/domain modules. Trigger for `Currency`, `CurrencyCategory` models, vendra-currency migrations, factories, seeders, policies, permission enums, Filament resources, clusters, forms, tables, relation managers, translations, media collections, plugin/service provider wiring, and module configuration."
---
...
Treat `app-modules/vendra-currency` as the source of currency domain behavior and Filament admin UI.
...
Follow the existing `Currency` and `CurrencyCategory` patterns for new currency entities.
...
- Run module checks from the package when possible: `composer --working-dir=app-modules/vendra-currency test` and `composer --working-dir=app-modules/vendra-currency analyse`.
