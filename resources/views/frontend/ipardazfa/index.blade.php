@extends('frontend.ipardazfa.layout.app', ['pageTitle' => __('خرید')])

@section('content')
    <section class="bg-white dark:bg-gray-800 lg:bg-[url('https://coin-payment.test/templates/coin-payment/header.png')] lg:bg-contain lg:bg-no-repeat lg:ltr:bg-right lg:rtl:bg-left">
        <div class="mx-auto max-w-screen-xl place-content-start px-4 py-8 lg:py-48">
            ...
            <a class="bg-primary-700 focus:ring-primary-300 dark:focus:ring-primary-900 mr-3 inline-flex items-center justify-center rounded-lg px-5 py-3 text-center text-base font-medium text-white focus:ring-4 dark:bg-yellow-400 dark:text-white" href="#">
                {{ __('خرید گیفت کارت') }}
            </a>
            <a class="inline-flex items-center justify-center rounded-lg border border-gray-300 px-5 py-3 text-center text-base font-medium text-gray-900 hover:bg-gray-100 focus:ring-4 focus:ring-gray-100 dark:border-gray-700 dark:text-white dark:hover:bg-gray-700 dark:focus:ring-gray-800" href="#">
                {{ __('مشاوره') }}
            </a>
        </div>
    </section>
    ...
    <!-- Stats -->
    <section class="bg-white saturate-100 dark:bg-yellow-400">
        <div class="mx-auto max-w-screen-xl px-4 py-8 text-center lg:px-6 lg:py-16">
            <dl class="mx-auto grid max-w-screen-md gap-8 text-gray-900 dark:text-white sm:grid-cols-3">
                <div class="flex flex-col items-center justify-center">
                    <dt class="mb-2 text-3xl font-black oldstyle-nums dark:text-gray-800 md:text-4xl">73M+</dt>
                    <dd class="text-xl font-black text-gray-900 dark:text-gray-800">developers</dd>
                </div>
                <div class="flex flex-col items-center justify-center">
                    <dt class="mb-2 text-3xl font-black oldstyle-nums dark:text-gray-800 md:text-4xl">1B+</dt>
                    <dd class="text-lg font-black text-gray-900 dark:text-gray-800">contributors</dd>
                </div>
                <div class="flex flex-col items-center justify-center">
                    <dt class="mb-2 text-lg font-black oldstyle-nums dark:text-gray-800 md:text-4xl">500 نفر</dt>
                    <dd class="text-2xl font-black text-gray-900 dark:text-gray-800">{{ __('تعداد مشتریان') }}</dd>
                </div>
            </dl>
        </div>
    </section>
                    ...
                    @foreach ($products as $item)
                        <a class="group" href="{{ route('products.show', [$item->token, $item->slug]) }}">
                            <div class="aspect-h-1 aspect-w-1 xl:aspect-h-5 xl:aspect-w-8 w-full overflow-hidden">
                                <img alt="{{ $item->name }}" class="h-full w-full origin-top-left rotate-12 object-cover saturate-50 group-hover:opacity-75" src="{{ $item->productImage }}" title="{{ $item->name }}">
                            </div>
                ...
                <div class="lg:-mx-6 lg:flex">
                    <div class="lg:w-3/4 lg:px-6">
                        <a href="{{ route('blogs.posts.show', $blogPosts->first()->slug) }}">
                            <img alt="" class="h-80 w-full rounded-xl object-cover object-center saturate-50 xl:h-[28rem]" src="{{ $blogPosts->first()->getMedia('blogs/posts') }}">
