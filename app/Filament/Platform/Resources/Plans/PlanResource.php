...
final class PlanResource extends Resource
{
    ...
    {
        return __('platform.plan');
    }

    public static function getPluralModelLabel(): string
    {
        return __('platform.plans');
    }
        ...
        return $schema
            ->components([
                ...
                TextInput::make('max_websites')
                    ->label(__('platform.max_websites'))
                    ->integer()
                    ->minValue(1)
                    ->required(),

                Select::make('period_unit')
                    ->label(__('platform.period_unit'))
                    ->native(false)
                    ->options(self::periodUnitOptions())
                    ->required(),

                TextInput::make('period_count')
                    ->label(__('platform.period_count'))
                    ->integer()
                    ->minValue(1)
                    ->required(),

                TextInput::make('grace_days')
                    ->label(__('platform.grace_days'))
                    ->integer()
                    ->minValue(0)
                    ->default(0)
                    ->required(),
                ...
                TextInput::make('price')
                    ...
                    ->helperText(__('platform.price_hint'))
                    ->integer()
                    ->minValue(0)
                    ->default(0)
                    ->live()
