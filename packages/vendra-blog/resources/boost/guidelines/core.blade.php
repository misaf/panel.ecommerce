...
- Integrate optional post tags only through `HasOptionalTags` and `TagIntegration` from `misaf/vendra-support`. Reserve the `blog` type; never import Vendra Tagger or Spatie Tags, and keep Tagger in Composer `suggest` rather than `require`.
...
- Keep Pest architecture tests in `tests/ArchTest.php`: the `php`, `security`, and `laravel` presets plus a tenant-agnostic expectation, e.g. `arch()->expect('Misaf\VendraBlog')->not->toUse('Misaf\VendraTenant')`.
