...
The `misaf/vendra-geo` package owns geographic reference data (countries, states, provinces, cities) and the Filament admin UI for countries, states, and cities.
...
- Keep geo domain code inside `packages/vendra-geo` using the `Misaf\VendraGeo` namespace.
...
- Store Geo `name` and `slug` values as plain strings, matching the Currency package pattern. Geo has no translated `description` field. Do not use Spatie Translatable, Lara Zeus translatable resource/page concerns, locale switchers, `InteractsWithTranslatedTableRecords`, or translated table callbacks in this package.
- Because the package resources declare a `$cluster`, keep their complete resource trees under `src/Filament/Clusters/Resources/`, use the matching `Misaf\VendraGeo\Filament\Clusters\Resources` namespace, and keep plugin discovery aligned. Any future resource without a cluster must instead live under `src/Filament/Resources/`.
...
- Keep Pest architecture tests in `tests/ArchTest.php`: the `php`, `security`, and `laravel` presets plus a tenant-agnostic expectation, e.g. `arch()->expect('Misaf\VendraGeo')->not->toUse('Misaf\VendraTenant')`.
