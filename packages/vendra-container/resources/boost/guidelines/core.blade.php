...
- **This package must not know what a Store, Reseller, Storefront, or tenant is.** It has no Vendra dependencies at all, and must not gain one. If a change here needs a business concept, the concept belongs in the caller and the change belongs in its `ContainerDefinition`.
...
- `ValueObjects\ResourceLimits` holds CPU/memory caps in operator units (cores, megabytes); the adapter converts them to nano-CPUs and bytes. An unset limit means uncapped, and an unconfigured set emits **no** resource keys at all — the Engine reads a zero as "unlimited", so a default zero would cap by accident.
