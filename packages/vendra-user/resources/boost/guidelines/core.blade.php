...
- `User` carries a nullable `account_id` (a billing account owned by `misaf/vendra-subscription`, exposed via `isAccountOwner()`). `canAccessPanel()` gates the app's panels by this for `account` and by roles for `admin`/`user`. Platform operators belong to the host application's separate authentication provider and must never be represented by this model. Keep `account_id` a plain nullable column with no relation here (no dependency on the subscription module).
