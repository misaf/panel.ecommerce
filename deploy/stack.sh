...
#   bin/stack property sync                          # re-render from properties.yml
#
# Properties are scaffolded from property-template/ into properties/<slug>/ (which
# is gitignored — no generated or secret material is committed). Their durable
# definition lives in the committed properties.yml registry, so a rebuilt host
# recovers the whole fleet with `property sync`.
#
set -euo pipefail
...
REGISTRY="$DEPLOY_DIR/properties.yml"
...
cmd_status() {
  ...
  local svc st
  for svc in php php-api mysql redis horizon; do
...
# ---- property registry (properties.yml) -------------------------------------
#
# The committed source of truth for the fleet. properties/<slug>/ is generated
# and gitignored, so without this a rebuilt host would not know what to run.
# Deliberately parsed with awk rather than yq: the schema is three flat keys and
# the stack should not need a YAML binary installed to recover itself.

# Emit "slug<TAB>domain<TAB>image" for each registered property.
registry_entries() {
  [[ -f "$REGISTRY" ]] || return 0
  awk '
    /^[[:space:]]*#/ { next }
    /^[[:space:]]*-[[:space:]]*slug:[[:space:]]*/ {
      if (slug != "") print slug "\t" domain "\t" image
      slug = $0; sub(/^[[:space:]]*-[[:space:]]*slug:[[:space:]]*/, "", slug)
      domain = ""; image = ""; next
    }
    /^[[:space:]]*domain:[[:space:]]*/ {
      domain = $0; sub(/^[[:space:]]*domain:[[:space:]]*/, "", domain); next
    }
    /^[[:space:]]*image:[[:space:]]*/ {
      image = $0; sub(/^[[:space:]]*image:[[:space:]]*/, "", image); next
    }
    END { if (slug != "") print slug "\t" domain "\t" image }
  ' "$REGISTRY" | sed 's/\r//g'
}

registry_has() { registry_entries | cut -f1 | grep -qx "$1"; }

registry_add() {
  local slug="$1" domain="$2" image="$3"
  [[ -f "$REGISTRY" ]] || printf 'properties: []\n' > "$REGISTRY"
  # First real entry replaces the empty-list placeholder.
  if grep -qE '^properties:[[:space:]]*\[\][[:space:]]*$' "$REGISTRY"; then
    sed -i.bak 's/^properties:[[:space:]]*\[\][[:space:]]*$/properties:/' "$REGISTRY"
    rm -f "$REGISTRY.bak"
  fi
  grep -qE '^properties:' "$REGISTRY" || printf 'properties:\n' >> "$REGISTRY"
  cat >> "$REGISTRY" <<EOF
  - slug: ${slug}
    domain: ${domain}
    image: ${image}
EOF
  info "registered '${slug}' in $(basename "$REGISTRY") — commit it"
}
...
registry_rm() {
  ...
  awk -v target="$slug" '
    /^[[:space:]]*-[[:space:]]*slug:[[:space:]]*/ {
      s = $0; sub(/^[[:space:]]*-[[:space:]]*slug:[[:space:]]*/, "", s)
      skip = (s == target); if (skip) next
    }
    skip && /^[[:space:]]+[a-z]+:[[:space:]]*/ { next }
    { skip = 0; print }
  ' "$REGISTRY" > "$tmp" && mv "$tmp" "$REGISTRY"
  # Collapse back to an explicit empty list so the file stays valid YAML.
  if ! registry_entries | grep -q .; then
    printf 'properties: []\n' > "$tmp"
    awk '/^properties:/ { exit } { print }' "$REGISTRY" > "$tmp.head" 2>/dev/null || true
    cat "$tmp.head" > "$REGISTRY" 2>/dev/null || true
    printf 'properties: []\n' >> "$REGISTRY"
    rm -f "$tmp" "$tmp.head"
  fi
  info "removed '${slug}' from $(basename "$REGISTRY") — commit it"
}

# Render properties/<slug>/ for every registry entry that has no directory yet.
# This is the recovery path on a fresh host.
property_render() {
  local slug="$1" domain="$2" image="$3" dir="$PROPERTIES_DIR/$slug"
  mkdir -p "$dir"
  cp "$TEMPLATE_DIR/docker-compose.yml" "$dir/docker-compose.yml"
  if [[ ! -f "$dir/.env" ]]; then
    cat > "$dir/.env" <<EOF
# Storefront property "${slug}" — generated by $PROG from properties.yml.
# Not committed. Re-create with: $PROG property sync
DOMAIN=${domain}
ROUTER_NAME=${slug}
...
# Path Traefik health-checks before routing to a replica.
...
property_sync() {
  local any="" slug domain image
  while IFS=$'\t' read -r slug domain image; do
    [[ -n "$slug" ]] || continue
    any=1
    if [[ -d "$PROPERTIES_DIR/$slug" ]]; then
      printf '  %-20s %s (already present)\n' "$slug" "$domain"
    else
      property_render "$slug" "$domain" "$image"
      printf '  %-20s %s %srendered%s\n' "$slug" "$domain" "$c_green" "$c_off"
    fi
  done < <(registry_entries)
  [[ -n "$any" ]] || { echo "(registry is empty — $PROG property add <slug> <domain> [image])"; return; }
  generate_local_cert force
  reload_proxy_cert
  info "sync complete — start them with '$PROG property up <slug>'"
}
...
property_add() {
  ...
  registry_has "$slug" && { warn "property '$slug' is already in $(basename "$REGISTRY")"; exit 1; }
  property_render "$slug" "$domain" "$image"
  registry_add "$slug" "$domain" "${image:-REPLACE_WITH_YOUR_NEXTJS_IMAGE}"
  info "created property '${slug}' → ${domain}  ($dir)"
  generate_local_cert force
  reload_proxy_cert
  echo "  next:"
  [[ -z "$image" ]] && echo "    • set STOREFRONT_IMAGE in ${dir}/.env and in $(basename "$REGISTRY")"
  echo "    • commit $(basename "$REGISTRY")   (the fleet's only durable record)"
...
property_rm() {
  ...
  registry_has "$slug" && registry_rm "$slug"
  info "removed property ${slug}"
  generate_local_cert force
  reload_proxy_cert
}

# Lists the REGISTRY (the durable definition), flagging entries whose directory
# has not been rendered on this host yet.
property_list() {
  local any="" slug domain image state
  while IFS=$'\t' read -r slug domain image; do
    [[ -n "$slug" ]] || continue
    any=1
    if [[ -d "$PROPERTIES_DIR/$slug" ]]; then state=""; else state="  ${c_yellow}(not rendered — $PROG property sync)${c_off}"; fi
    printf '  %-20s %s%s\n' "$slug" "$domain" "$state"
  done < <(registry_entries)
  # Anything on disk but absent from the registry is a drift bug worth surfacing.
  if [[ -d "$PROPERTIES_DIR" ]]; then
    for env in "$PROPERTIES_DIR"/*/.env; do
      [[ -f "$env" ]] || continue
      slug="$(basename "$(dirname "$env")")"
      registry_has "$slug" && continue
      any=1
      printf '  %-20s %s  %s(unregistered — add it to %s)%s\n' \
        "$slug" "$(env_value "$env" DOMAIN)" "$c_red" "$(basename "$REGISTRY")" "$c_off"
    done
  fi
...
cmd_property() {
  ...
  case "$sub" in
    ...
    sync)     property_sync;;
    *) warn "usage: property {add <slug> <domain> [image] | up|down|restart|rm <slug> | ls | sync}"; exit 1;;
  esac
}
...
Usage: $PROG <command>
  ...
  property ls                            list properties (from properties.yml)
  property sync                          render properties/ from properties.yml
EOF
}
